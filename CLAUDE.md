...
- `hardware.py` — `patch_device_info()` seeds the `_DEVICES` cache with corrected `Capabilities` (`CleanMower` + `GetCleanInfo` instead of `GetCleanInfoV2`). Uses the library's own caching mechanism instead of monkeypatching. `SUPPORTED_CLASSES` lists verified device classes (`2i0fns` = O1200 LiDAR Pro, `9bts2s` and `2px96q` = O800 RTK, `77atlz` = G1-800).
