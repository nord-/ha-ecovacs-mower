...
async def test_refresh_commands_empty_for_unknown_events() -> None:
    ...
    # not raise.
    #
    # For MowerProtectStateEvent the empty list is also the reason the rain
    # sensor asks for GetProtectState itself: request_refresh() has nothing to
    # send for an event the library does not know.
    ...
    assert static.capabilities.get_refresh_commands(MowerProtectStateEvent) == []
