...
def parse_map_trace(blob: bytes) -> CoveredArea:
    ...
    **While a job runs the firmware moves the live outline into section
    3** and leaves section 1 holding the batch id alone — one record per
    run, newest first, renumbered as runs are added (GOAT G1-800, fw
    1.36.208, issue #52). Not a firmware dialect: the same device sent
    the classic section-1 form eighteen seconds before that job started.
    Section 3 was empty in all 3660 blobs of the issue-41 captures, and
    those were all taken on an idle mower, which is why it read as unused.
    The two sections share a coordinate frame and a dialect — an idle
    snapshot and a mid-job record of the same capture carry a verbatim
