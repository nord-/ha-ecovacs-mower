...
Run: `python -m pytest tests/deebot_patch/ tests/test_map.py tests/test_map_svg.py -p no:homeassistant -v`
Expected: all PASS (deebot-client-importing files may error on import locally if the library is absent — those are CI's responsibility; do not mark them skipped to silence local runs)
